...
def load_to_database(
    transformed_data: Dict[str, Tuple], db: Session, batch_size: int = 1000
    ...
    if "order_items" in transformed_data:
        ...
        stats["order_items"] = _load_table_composite(
            df, OrderItem, db, batch_size, ["order_id", "order_item_id"]
        )

    # Carrega order_payments
    if "order_payments" in transformed_data:
        df, erros, avisos = transformed_data["order_payments"]
        stats["order_payments"] = _load_table_composite(
            df, OrderPayment, db, batch_size, ["order_id", "payment_sequential"]
        )
...
def _load_table(df, model_class, db: Session, batch_size: int, id_column: str) -> Dict[str, int]:
    """
    ...
    """
    stats = {"total_registros": len(df), "carregados": 0, "duplicatas": 0, "erros": 0}
    ...
    for i in range(0, len(records), batch_size):
        ...
        try:
            ...
            for id_val in ids:
                existing = (
                    db.query(model_class).filter(getattr(model_class, id_column) == id_val).first()
                )
                if existing:
            ...
            logger.info(
                f"Batch {i//batch_size + 1} ({model_class.__tablename__}): {len(novos_registros)} novos registros"
            )

        except Exception as e:
            logger.error(
                f"Erro ao carregar batch {i//batch_size + 1} ({model_class.__tablename__}): {str(e)}"
            )
            db.rollback()
...
def _load_table_composite(
    df, model_class, db: Session, batch_size: int, id_columns: List[str]
) -> Dict[str, int]:
    """
