...
st.set_page_config(
    page_title="Olist Analytics", page_icon="📊", layout="wide", initial_sidebar_state="expanded"
)
...
def page_header(title, sub=""):
    ...
    st.markdown(f'<div class="page-hdr"><h1>{title}</h1>{s}</div>', unsafe_allow_html=True)
...
def chart_style(fig, height=340):
    fig.update_layout(
        ...
        legend=dict(font=dict(color=SUB), bgcolor="rgba(0,0,0,0)", bordercolor="rgba(0,0,0,0)"),
...
def ask_ia(q):
    try:
        r = requests.post(f"{API_URL}/api/v1/ia/perguntar", json={"pergunta": q}, timeout=35)
        return r.json() if r.status_code == 200 else None
    except Exception:
        return None
...
def gradient_selector_card(current_value: float) -> str:
    ...
    for i, t in enumerate(tiers):
        ...
        sz = t["size"]
        ring = (
            f'<div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:{sz+14}px;height:{sz+14}px;border-radius:50%;border:1px dashed {t["color"]}44;"></div>'
            if active
            else ""
        )
        circle = (
        ...
        if i < len(tiers) - 1:
            line_bg = (
                f"linear-gradient(90deg,{t['color']},{t['next']})" if i < reached - 1 else "#1e2860"
            )
            track += (
                f'<div style="flex:1;height:{2+i}px;background:{line_bg};'
...
if pagina == "Dashboard":
    ...
    if kpis:
        ...
        if kpis.get("receita_por_mes"):
            ...
            df_mes["periodo"] = (
                df_mes["ano"].astype(str) + "-" + df_mes["mes"].astype(str).str.zfill(2)
            )
            ...
            if len(periodos) > 1:
                ...
                with c2:
                    fim = st.selectbox("Até", periodos, index=len(periodos) - 1, key="d_fim")
            ...
            with col1:
                ...
                fig = go.Figure(
                    go.Scatter(
                        ...
                        marker=dict(size=7, color=ACC, line=dict(color=CARD_SOLID, width=2)),
                        fill="tozeroy",
                        fillcolor="rgba(92,111,245,0.09)",
            ...
            with col2:
                ...
                if kpis.get("receita_por_estado"):
                    ...
                    fig = px.bar(
                        ...
                        y="receita",
                        color="receita",
                        color_continuous_scale=[[0, "rgba(92,111,245,0.3)"], [1, ACC]],
                    )
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    name="Acumulado",
                    mode="lines",
                    line=dict(color=ACC, width=2.5, dash="dot"),
                    fill="tozeroy",
                    fillcolor="rgba(92,111,245,0.06)",
...
# ── Página: Produtos ───────────────────────────────────────────────────────────
    ...
    if kpis:
        top_n = st.slider("Exibir top", min_value=5, max_value=20, value=10, step=5, key="top_n")
        ...
        with col1:
            ...
            if kpis.get("top_produtos"):
                ...
                fig.update_traces(
                    hovertemplate="<b>%{y}</b><br>R$ %{x:,.2f}<extra></extra>", marker_line_width=0
                )
                st.plotly_chart(
                    chart_style(fig, height=max(280, top_n * 28)), use_container_width=True
                )
        ...
        with col2:
            ...
            if kpis.get("top_categorias"):
                ...
                fig = px.bar(
                    ...
                    orientation="h",
                    color="faturamento",
                    color_continuous_scale=[[0, "rgba(168,85,247,0.3)"], [1, ACC2]],
                )
                fig.update_coloraxes(showscale=False)
                fig.update_yaxes(categoryorder="total ascending")
                fig.update_traces(
        ...
        with col2:
            ...
            if kpis.get("top_categorias"):
                ...
                fig.update_coloraxes(showscale=False)
                fig.update_traces(
                    hovertemplate="<b>%{x}</b><br>%{y} unidades<extra></extra>", marker_line_width=0
                )
                st.plotly_chart(chart_style(fig), use_container_width=True)
        ...
        if kpis.get("top_categorias"):
            with st.expander("📋  Ver dados completos por categoria"):
                ...
                df_t["faturamento"] = df_t["faturamento"].apply(lambda x: f"R$ {x:,.2f}")
...
# ── Página: Clientes e Geografia ───────────────────────────────────────────────
    ...
    page_header("Clientes e Geografia", "Distribuição geográfica e comportamento de compra")
    kpis = get_kpis()
    ...
    if kpis:
        ...
        with col1:
            ...
            if kpis.get("pedidos_por_estado"):
                ...
                fig.update_coloraxes(showscale=False)
                fig.update_traces(
                    hovertemplate="<b>%{x}</b><br>%{y} pedidos<extra></extra>", marker_line_width=0
                )
                st.plotly_chart(chart_style(fig), use_container_width=True)
        ...
        with col2:
            ...
            if kpis.get("receita_por_estado"):
                ...
                fig.update_coloraxes(showscale=False)
                fig.update_traces(
                    hovertemplate="<b>%{x}</b><br>R$ %{y:,.2f}<extra></extra>", marker_line_width=0
                )
                st.plotly_chart(chart_style(fig), use_container_width=True)
        ...
        if kpis.get("metodos_pagamento"):
            ...
            with col2:
                ...
                for _, row in df_pay.sort_values("valor_total", ascending=False).iterrows():
...
# ── Página: Assistente IA ──────────────────────────────────────────────────────
    ...
    page_header("Assistente de Dados", "Perguntas em linguagem natural sobre o dataset Olist")
