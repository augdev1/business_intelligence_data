...
class VendaRepository(BaseRepository[Venda]):
    ...
    def get_produto_mais_vendido(self) -> Optional[Dict[str, Any]]:
        ...
        """
        result = (
            self.db.query(Venda.produto, func.sum(Venda.quantidade).label("total_quantidade"))
    ...
    def get_categoria_mais_vendida(self) -> Optional[Dict[str, Any]]:
        ...
        """
        result = (
            self.db.query(Venda.categoria, func.sum(Venda.quantidade).label("total_quantidade"))
    ...
    def get_cidade_maior_faturamento(self) -> Optional[Dict[str, Any]]:
        ...
        """
        result = (
            self.db.query(Venda.cidade, func.sum(Venda.faturamento).label("total_faturamento"))
    ...
    def get_evolucao_mensal(self) -> List[Dict[str, Any]]:
        ...
        return [
            {"ano": int(r[0]), "mes": int(r[1]), "faturamento": r[2], "quantidade": r[3]}
            for r in results
        ]
    ...
    def get_ranking_produtos(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        return [{"produto": r[0], "quantidade": r[1], "faturamento": r[2]} for r in results]
    ...
    def get_ranking_cidades(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        return [{"cidade": r[0], "quantidade": r[1], "faturamento": r[2]} for r in results]
    ...
    def get_vendas_por_periodo(self, data_inicio: date, data_fim: date) -> List[Venda]:
        ...
        return (
            self.db.query(Venda)
            .filter(and_(Venda.data_venda >= data_inicio, Venda.data_venda <= data_fim))
            .all()
        )
