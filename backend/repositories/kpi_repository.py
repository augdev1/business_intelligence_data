...
class KPIRepository:
    ...
    def get_receita_total(self) -> Decimal:
        ...
        Returns:
            Receita total
        """
        result = self.db.query(func.sum(OrderItem.price + OrderItem.freight_value)).scalar()
    ...
    def get_receita_por_estado(self) -> List[Dict[str, Any]]:
        ...
        return [{"estado": r[0], "receita": r[1]} for r in results]
    ...
    def get_receita_por_mes(self) -> List[Dict[str, Any]]:
        ...
        return [{"ano": int(r[0]), "mes": int(r[1]), "receita": r[2]} for r in results]
    ...
    def get_top_categorias(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        return [
            {"categoria": r[0], "faturamento": r[1], "receita_total": r[2], "quantidade": r[3]}
            for r in results
            if r[0]
        ]
    ...
    def get_metodos_pagamento(self) -> List[Dict[str, Any]]:
        ...
        return [{"tipo": r[0], "quantidade": r[1], "valor_total": r[2]} for r in results]
    ...
    def get_pedidos_por_estado(self) -> List[Dict[str, Any]]:
        ...
        """
        results = (
            self.db.query(Customer.customer_state, func.count(Order.order_id).label("quantidade"))
            .join(Order, Customer.customer_id == Order.customer_id)
            .group_by(Customer.customer_state)
        ...
        return [{"estado": r[0], "quantidade": r[1]} for r in results]
